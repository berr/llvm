...
static RegisterRegAlloc
  aiRegAlloc("ai", "register allocator using AI", createAIRegisterAllocator);

namespace {
  class RAIA : public MachineFunctionPass {
  public:
    static char ID;
    RAIA() : MachineFunctionPass(ID), StackSlotForVirtReg(-1),
...
int RAIA::getStackSpaceFor(unsigned VirtReg, const TargetRegisterClass *RC) {
...
///
bool RAIA::isLastUseOfLocalReg(MachineOperand &MO) {
...
void RAIA::addKillFlag(const LiveReg &LR) {
...
void RAIA::killVirtReg(LiveRegMap::iterator LRI) {
  addKillFlag(*LRI);
...
/// killVirtReg - Mark virtreg as no longer available.
void RAIA::killVirtReg(unsigned VirtReg) {
...
void RAIA::spillVirtReg(MachineBasicBlock::iterator MI, unsigned VirtReg) {
  assert(TargetRegisterInfo::isVirtualRegister(VirtReg) &&
...
void RAIA::spillVirtReg(MachineBasicBlock::iterator MI,
...
void RAIA::spillAll(MachineBasicBlock::iterator MI) {
...
void RAIA::usePhysReg(MachineOperand &MO) {
...
void RAIA::definePhysReg(MachineInstr *MI, unsigned PhysReg,
                           RegState NewState) {
  markRegUsedInInstr(PhysReg);
...
unsigned RAIA::calcSpillCost(unsigned PhysReg) const {
...
///
void RAIA::assignVirtToPhysReg(LiveReg &LR, unsigned PhysReg) {
...
RAIA::LiveRegMap::iterator
RAIA::assignVirtToPhysReg(unsigned VirtReg, unsigned PhysReg) {
  LiveRegMap::iterator LRI = findLiveVirtReg(VirtReg);
...
RAIA::LiveRegMap::iterator RAIA::allocVirtReg(MachineInstr *MI,
                                                  LiveRegMap::iterator LRI,
                                                  unsigned Hint) {
...
RAIA::LiveRegMap::iterator
RAIA::defineVirtReg(MachineInstr *MI, unsigned OpNum,
                      unsigned VirtReg, unsigned Hint) {
  assert(TargetRegisterInfo::isVirtualRegister(VirtReg) &&
...
RAIA::LiveRegMap::iterator
RAIA::reloadVirtReg(MachineInstr *MI, unsigned OpNum,
                      unsigned VirtReg, unsigned Hint) {
  assert(TargetRegisterInfo::isVirtualRegister(VirtReg) &&
         "Not a virtual register");
  LiveRegMap::iterator LRI;
...
bool RAIA::setPhysReg(MachineInstr *MI, unsigned OpNum, unsigned PhysReg) {
  MachineOperand &MO = MI->getOperand(OpNum);
  bool Dead = MO.isDead();
  if (!MO.getSubReg()) {
    MO.setReg(PhysReg);
...
void RAIA::handleThroughOperands(MachineInstr *MI,
                                   SmallVectorImpl<unsigned> &VirtDead) {
...
///
bool RAIA::runOnMachineFunction(MachineFunction &Fn) {
